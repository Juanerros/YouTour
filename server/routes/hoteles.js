...
router.post('/', async (req, res) => {
    const { nombre, id_ciudad, rating, precio_noche, descripcion, amenidades } = req.body;
    ...
    try {
        ...
        const [result] = await conex.execute(
            ...
            [nombre, id_ciudad, rating, precio_noche, descripcion, amenidades]
        );
        
        res.status(201).json({
            id_hotel: result.insertId,
            nombre,
            id_ciudad,
            rating,
            precio_noche,
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        const [result] = await conex.execute(
            'UPDATE hoteles SET nombre = ?, id_ciudad = ?, rating = ?, precio_noche = ?, descripcion = ?, amenidades = ? WHERE id_hotel = ?',
            [nombre, id_ciudad, rating, precio_noche, descripcion, amenidades, id]
        );
